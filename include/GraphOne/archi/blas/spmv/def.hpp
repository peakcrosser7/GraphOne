...
struct SpmvCsrParams {
    ...
    const offset_t*       row_offsets;       ///< Pointer to the array of \p m offsets demarcating the end of every row in \p d_column_indices and \p d_values
    const index_t*        col_indices;       ///< Pointer to the array of \p num_nonzeros column-indices of the corresponding nonzero elements of matrix <b>A</b>.  (Indices are zero-valued.)
    const mat_value_t*    values;            ///< Pointer to the array of \p num_nonzeros values of the corresponding nonzero elements of matrix <b>A</b>.
    const vec_x_value_t*  vector_x;          ///< Pointer to the array of \p num_cols values corresponding to the dense input vector <em>x</em>
...
MakeCsrSpMV(index_t n_rows, index_t n_cols, offset_t nnz,
    ...
    SpmvCsrParams<index_t, offset_t, mat_value_t, vec_x_value_t, vec_y_value_t> params;
    params.n_rows      = n_rows;
    params.n_cols      = n_cols;
    params.nnz         = nnz;
    params.row_offsets = row_offsets; // const_cast<offset_t *>(row_offsets);
    params.col_indices = col_indices; // const_cast<index_t *>(col_indices);
    params.values      = values; // const_cast<mat_value_t *>(values);
    params.vector_x    = vector_x; // const_cast<vec_x_value_t *>(vector_x);
